export type EncryptResponseType = {
  kx: string;
  ky: string;
  kz: number;
};
...
export default class E2EE {
   ...
   * @returns {EncryptResponseType}
   */
  public encrypt(plainText: string, key?: number, padding?: string): EncryptResponseType {
    const minKey = 2;
    ...
    const encryptedText = btoa(btoa(rows.map((row) => row.join("")).join("")));
    return { kx: encryptedText, ky: padding, kz: key };
  }
  ...
  public decrypt(encryptedText: string, key: number, padding?: string): string {
    try {
      ...
      encryptedText = atob(atob(encryptedText));
